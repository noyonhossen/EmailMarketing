...
namespace EmailMarketing.Framework.Tests.Services.Contacts
{
    ...
    public class FieldMapServiceTests
    {
        ...
        [Test]
        public void DeleteAsync_FieldMapIdExists_DeleteFieldMap()
        {
            //Arrange
            var fieldMap = new FieldMap
            {
            ...
            _fieldMapRepositoryMock.Setup(x => x.GetByIdAsync(id)).ReturnsAsync(fieldMap).Verifiable();

            _fieldMapRepositoryMock.Setup(x => x.DeleteAsync(fieldMap.Id)).Returns(Task.CompletedTask).Verifiable();
            _fieldMapUnitOfWorkMock.Setup(x => x.SaveChangesAsync()).Returns(Task.CompletedTask).Verifiable();

            //Act 
            var result = _fieldMapService.DeleteAsync(id);

            //Assert
             result.Result.ShouldBe(fieldMap);
            _fieldMapRepositoryMock.VerifyAll();
            _fieldMapUnitOfWorkMock.VerifyAll();
            
        }
        [Test]
        public void DeleteAsync_FieldMapIdDoesNotExists_ThrowExceptions()
        {
            //Arrange
            var fieldMap = new FieldMap
            {
            ...
            _fieldMapRepositoryMock.Setup(x => x.GetByIdAsync(fieldMap.Id)).ReturnsAsync(fieldmap).Verifiable();


            //Act 
            Should.Throw<NotFoundException>(() =>
               _fieldMapService.DeleteAsync(fieldMap.Id)
          );
